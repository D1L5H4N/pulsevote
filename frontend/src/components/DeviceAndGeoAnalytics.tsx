      ...
      <div className="rounded-xl border border-slate-800/80 bg-slate-900/60 p-5 backdrop-blur-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-semibold text-white text-sm">Device Distribution</h3>
              <p className="text-[11px] text-slate-500">Breakdown of voter clients</p>
            </div>
          </div>
        ...
        <div className="space-y-2.5">
          {deviceItems.map((item) => (
            <div
              key={item.name}
              className="flex items-center justify-between p-2.5 rounded-lg bg-slate-950/40 border border-slate-800/30 text-xs"
            >
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 rounded-md bg-slate-800/60 flex items-center justify-center">
                  {item.icon}
                </div>
                <div>
                  <span className="text-white font-medium">{item.name}</span>
                  <span className="text-slate-500 ml-1.5 font-mono">({item.count})</span>
                </div>
              </div>
      ...
      <div className="rounded-xl border border-slate-800/80 bg-slate-900/60 p-5 backdrop-blur-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-semibold text-white text-sm">Geographic Insights</h3>
              <p className="text-[11px] text-slate-500">Country aggregation (privacy-safe)</p>
            </div>
          </div>
              ...
              <div key={geo.country} className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2 font-medium text-slate-200">
                    <span className="w-5 h-5 rounded bg-slate-800 text-[10px] font-bold flex items-center justify-center text-slate-400 font-mono">
